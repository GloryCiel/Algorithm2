...
def grahamScan(points):
    ...
    pointsWithAngle=[(x,y,math.atan2(y-start[1],x-start[0])) for x, y in points]#각도 포함해서 3-tuple
    res=[]


    pointsWithAngle = sorted(pointsWithAngle, key=lambda p: (p[2],p[1],-p[0]))#각도순으로 정렬된 점들
    for p in pointsWithAngle:
        res.append((p[0],p[1]))
        while len(res)>2 and not ccw(res[-3],res[-2],res[-1]):
            res.pop(-2)
    return res
...
if __name__ == "__main__":
    input_points = [(3, -1), (2, -2), (4, -1)]
    print(grahamScan(input_points))  # Expected output: [(2, -2), (4, -1), (3, -1)]
